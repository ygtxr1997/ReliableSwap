...
from infoswap.preprocess.mtcnn_pytorch.src.get_nets import PNet, RNet, ONet
from infoswap.preprocess.mtcnn_pytorch.src.box_utils import nms, calibrate_box, get_image_boxes, convert_to_square
from infoswap.preprocess.mtcnn_pytorch.src.first_stage import run_first_stage
from infoswap.preprocess.mtcnn_pytorch.src.align_trans import get_reference_facial_points, warp_and_crop_face
...
class MTCNN(nn.Module):
    def __init__(self, square=True):
        ...
        self.onet.eval()
        # crop_size = (112, 112):
        self.reference = get_reference_facial_points(default_square=square)
    ...
    def align(self, img, crop_size=(112, 112), return_trans_inv=False):
        ...
        """
        # 取关键点列的前3个点的坐标, 计算Similarity Transform Matrix, 进行warp和crop
        """
        warped_face = warp_and_crop_face(np.array(img), facial5points, self.reference, crop_size=crop_size,
    ...
    def align_fully(self, img, crop_size=(112, 112), return_trans_inv=False, ori=[0, 1, 3], fast_mode=True):
        ...
        warped_face = warp_and_crop_face(np.array(ori_size), facial5points, self.reference, crop_size=crop_size,
                                         return_trans_inv=return_trans_inv)
        if return_trans_inv:
            return Image.fromarray(warped_face[0]), warped_face[1]
        else:
    ...
    def align_multi(self, img, limit=None, min_face_size=64.0, crop_size=(112, 112), thresholds=[0.6, 0.7, 0.8], factor=0.707, reverse=False, mode=None):
        boxes, landmarks = self.detect_faces(img, min_face_size, thresholds=thresholds, factor=factor)
        if len(landmarks) == 0:
            return None
        if limit:
            boxes = boxes[:limit]
        ...
        for landmark in landmarks:
            ...
            # print(facial5points)  # in the original image (un-cropped)
            # crop_size = (256, 256)
            warped_face, tfm_inv = warp_and_crop_face(np.array(img), facial5points, self.reference, crop_size=crop_size, return_trans_inv=True, mode=mode)
            faces.append(Image.fromarray(warped_face))

            tfm_invs.append(tfm_inv)
        if reverse:
            return faces, tfm_invs, boxes
        else:
            return faces
