...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        image1 = norm(torch.rand((1, 3, 224, 224)) * 255)
        yaw1, pitch1, roll1 = model(image1)
        print(yaw1.shape, pitch1.shape, roll1.shape)
        param1 = torch.cat((yaw1, pitch1, roll1), dim=-1).cpu().numpy()
        print(param1.shape)

        image2 = norm(torch.rand((1, 3, 224, 224)) * 255)
        image2 = image1 - 0.5
        yaw2, pitch2, roll2 = model(image2)
        param2 = torch.cat((yaw2, pitch2, roll2), dim=-1).cpu().numpy()

    from sklearn.metrics import mean_squared_error
    dist = mean_squared_error(param1, param2)
    dim = param1.shape[-1]
    print(np.sqrt(dist * dim))
