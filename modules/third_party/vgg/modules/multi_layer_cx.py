...
class MultiContextualLoss(nn.Module):
    """
    ...
    def __init__(
        ...
        super(MultiContextualLoss, self).__init__()
    ...
    def forward(self, x, y):
        if hasattr(self, "vgg_model"):
            ...
            cxloss = 0.0
            for vgg_l in self.vgg_layer:
                # picking up vgg feature maps
                x = getattr(self.vgg_model(x), vgg_l)
                y = getattr(self.vgg_model(y), vgg_l)
                cxloss += F.contextual_loss(
                    x, y, self.band_width, self.loss_type
                ).mean()
