...
def vid_preprocessing(vid_path):
    ...
    vid = vid_dict[0].permute(0, 3, 1, 2).unsqueeze(0)  # (T,H,W,C) -> (1,T,C,H,W)
    ...
    def run(self):
        ...
        with torch.no_grad():
            ...
            if self.args.model == 'ted':
                ...
                h_start = self.gen.enc.enc_motion(self.vid_target[:, 20, :, :, :])  # (1,T,C,H,W)

            # self.vid_target = self.vid_target[:, 20, :, :, :].unsqueeze(1)
            for i in tqdm(range(self.vid_target.size(1))):
                img_target = self.vid_target[:, i, :, :, :]  # (N,C,H,W)
                img_recon = self.gen(self.img_source, img_target, h_start)
                vid_target_recon.append(img_recon.unsqueeze(2))  # (N,C,H,W) to (N,C,1,H,W)
